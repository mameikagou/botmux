...
import type { CredentialBinding, PrincipalBinding } from './execution/podman-provider.js';
...
export interface Session {
  ...
  /** Frozen, non-secret OpenMemory entitlement for a native owner session.
   * Podman sessions carry the same fact inside `principalBinding`. */
  ownerCanOpenMemory?: boolean;
  /** V4 guest pod identity, frozen with the topic's execution decision. */
  sandboxUserId?: string;
  /** V4 pod generation, frozen so a stale topic cannot join a replacement pod. */
  podGeneration?: number;
  /** Frozen principal identity/capability supplied by the T6 boundary. */
  principalBinding?: PrincipalBinding;
  /** Frozen non-secret skill leaves supplied at new-topic ingress. */
  principalSkills?: FrozenPrincipalSkillBinding[];
