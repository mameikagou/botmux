...
**8. API-task Full Access 例外** — 上述 no-transport 文件隔离仍是默认值。只有 owner 在 bot 配置中显式设置 `apiTaskFullAccess: true` 后创建的 HTTP virtual/API-task session，才会冻结该能力并跳过默认文件隔离；`/api/trigger` 请求不能携带或补授该能力，普通聊天与旧 session 不受影响。bot 的 `readIsolation`、session 已冻结的 `sandbox` 与 host `BOTMUX_SANDBOX=1` 始终优先。启用意味着该 session 的 agent 进程拥有同一 OS 用户可见的 host 文件与 IPC 能力；这是为必须使用 host IPC 的受信 bot 明确放弃文件隔离，而不是路径 carve-out。
